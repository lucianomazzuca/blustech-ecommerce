...
describe("ProductRepository methods", () => {
  ...
  test('save throws an error because the parameter is not an instanceof Product', async () => {
    ...
    await expect(productRepository.save(product)).rejects.toThrowError(ProductNotDefinedError);
  });
  ...
  test('getById throws an error because the is no brand with that id', async () => {
    const id = 2;
    await expect(productRepository.getById(id)).rejects.toThrowError(ProductNotFoundError);
  })

  test('getById throws an error because the argument is empty', async () => {
    await expect(productRepository.getById()).rejects.toThrowError(ProductIdNotDefinedError);
  });
