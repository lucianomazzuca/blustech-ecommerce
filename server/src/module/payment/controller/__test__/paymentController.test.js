...
const mockPaymentController = new PaymentController({ paymentService: mockPaymentService });
...
describe("Payment Controller methods", () => {
  ...
  test("getMercadoPagoLink calls service's getItemsForMercadoPago, createPaymentMercadoPago and sends a link", async () => {
    ...
    expect(mockPaymentService.getItemsForMercadoPago).toHaveBeenCalledWith(ordersMock);
